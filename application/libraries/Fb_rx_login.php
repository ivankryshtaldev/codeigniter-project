...
{				
	...
	**********/
	...
	{
		$url="https://graph.facebook.com/v2.11/me/broadcast_reach_estimations?access_token={$post_access_token}&method=post";
		$ch = curl_init();
		$headers = array("Content-type: application/json");
	...
	{
		$url="https://graph.facebook.com/v2.11/{$reach_estimation_id}?access_token={$post_access_token}";
		$ch = curl_init();
		$headers = array("Content-type: application/json");
	...
	{
		$url = "https://graph.facebook.com/v2.8/me/insights/?metric=page_messages_total_messaging_connections,page_messages_new_conversations_unique,page_messages_blocked_conversations_unique,page_messages_reported_conversations_unique&access_token={$access_token}&since={$from_date}&until={$to_date}";
    	$ch = curl_init();
    	$headers = array("Content-type: application/json");
    	curl_setopt($ch, CURLOPT_URL, $url);
    	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	{
		$url="https://graph.facebook.com/v2.11/{$label_id}?access_token={$page_access_token}";
		return $this->call_api_post('',$url,true);
	}

	function retrieve_label($page_access_token='')
	{
		$url="https://graph.facebook.com/v2.11/me/custom_labels?fields=name&access_token={$page_access_token}&limit=200";
		return $this->call_api_post('',$url,false);
	}

	function retrieve_level_of_psid($psid,$page_access_token){

		$url="https://graph.facebook.com/v2.11/{$psid}/custom_labels?fields=name&access_token=$page_access_token&limit=200";
		return $this->call_api_post('',$url,false);
