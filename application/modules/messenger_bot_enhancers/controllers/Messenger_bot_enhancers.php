...
In this case, we suggest you to check the error message in report, and if you think it’s not for your message content, but for specific subscribers, you can restart the campaign from where it is left off, by clicking on the option menu & then click Force Resume. ").'"><i class="fas fa-info-circle"></i> </a>';
    ...
    public function campaign_sent_status()
    {
        ...
        $response .="<table id='campaign_report' class='table table-bordered nowrap'>";
        $response .= "<thead><tr>";
        $response .= "<th class='text-center'>{$this->lang->line("SL")}</th>";
        $response .= "<th>{$this->lang->line("First Name")}</th>";
        $response .= "<th>{$this->lang->line("Last Name")}</th>";
        $response .= "<th class='text-center'>{$this->lang->line("Subscriber ID")}</th>";
        $response .= "<th class='text-center'>{$this->lang->line("Sent at")}</th>";
        $response .= "<th class='text-center'>{$this->lang->line("Delivered at")}</th>";
        $response .= "<th class='text-center'>{$this->lang->line("Opened at")}</th>";
        $response .= "<th>{$this->lang->line("Sent ID")}</th>";
        $response .= "</tr></thead>";
        $i=0;

        foreach ($report as $key => $value)
        {
            if(!isset($value["subscribe_id"])) $value["subscribe_id"] = "";
            if(!isset($value["message_sent_id"])) $value["message_sent_id"] = "";
            if(!isset($value["subscriber_name"])) $value["subscriber_name"] = "";
            if(!isset($value["subscriber_lastname"])) $value["subscriber_lastname"] = "";
            if(!isset($value["sent_time"])) $value["sent_time"] = "";
            if(!isset($value["delivery_time"])) $value["delivery_time"] = "";
            if(!isset($value["open_time"])) $value["open_time"] = "";
            if(!isset($value["click_time"])) $value["click_time"] = "";
            if(!isset($value["click_ref"])) $value["click_ref"] = "";

            $sent=$opened=$clicked=$delivered="<i class='fa fa-remove red'></i>";
            if($value["sent"]=="1") $sent="<i class='fa fa-check-circle green'></i> ".date("M j, y H:i",strtotime($value['sent_time']));
            ...
            $subscriber_auto_id=$value["subscriber_auto_id"];
            $response .= "<tr>";
            $response .= "<td>".$i."</td>";
            $response .= "<td>".$value['subscriber_name']."</td>";
            $response .= "<td>".$value['subscriber_lastname']."</td>";

            $response .= "<td class='text-center'>".$value['subscribe_id']."</td>";

            $response .= "<td class='text-center'>".$sent."</td>";
            $response .= "<td class='text-center'>".$delivered."</td>";
            $response .= "<td class='text-center'>".$opened."</td>";
            
            if($value['message_sent_id']=="") $response.= "<td><i class='fa fa-remove red'></i></td>";
            else $response .= "<td>".$msg_sent_id."</td>";

            // $response .= "<th class='text-center'>".$clicked."</th>";
          }
    ...
    private function get_broadcast_tags()
    {
      $old_tags = array
      (
      ...
      if(strtotime(date("Y-m-d")) > strtotime("2020-1-15")) return $new_tags;

      unset($new_tags[""]);
      $dropdown = array(""=>$this->lang->line("Select Tag"),"New Tags"=>$new_tags,"Tags supported until Jan 15,2020"=>$old_tags);
      return $dropdown;
    ...
    public function subscriber_bulk_broadcast_add_action()
    {
        ...
        $insert_data['campaign_name'] = $this->input->post("campaign_name");
        ...
        $sql="SELECT * FROM messenger_bot_subscriber WHERE ".$sql_part." ".$sql_part2." ".$sql_part3." ".$promo_sql." user_id = ".$this->user_id." AND unavailable = '0' AND is_bot_subscriber='1' AND page_table_id = {$page_table_id}";
        ...
        {
          $sql_24h="UPDATE messenger_bot_subscriber SET is_24h_1_sent='1' WHERE ".$sql_part."  ".$sql_part2."  ".$sql_part3." ".$promo_sql." user_id = ".$this->user_id." AND unavailable = '0' AND is_bot_subscriber='1' AND page_table_id = {$page_table_id}";
        ...
        if($this->basic->insert_data('messenger_bot_broadcast_serial',$insert_data))
        {
            ...
            $this->_insert_usage_log($module_id=211,$request=1);
    ...
    public function get_campaign_report()
    {
        ...
        foreach ($template_data as $key => $value) 
        {
            ...
            $template_data_formatted[$value['id']]['link']= base_url('/messenger_bot/edit_template/'.$value['id'].'/1');
        }
    ...
    public function sequence_message_campaign($page_auto_id=0,$iframe='0')
    {
        ...
        $data['bot_settings'] = $this->basic->get_data("messenger_bot_drip_campaign",array("where"=>array("page_id"=>$page_auto_id,"user_id"=>$this->user_id)),$select='',$join='',$limit='',$start=NULL,$order_by='created_at DESC');
    ...
    public function edit_sequence_message_campaign($id=0,$page_auto_id=0,$iframe='0')
    {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(219,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function get_engagement_list()
    {
        ...
        echo '<div class="well text-justify" style="border:1px solid #6777ef;padding:15px;color:#6777ef;">'.$this->lang->line("Sequence message campaign will be assigned for checkbox plugin only after replying back of the message sent for checkbox OPTIN.").'</div><br>';
    ...
    public function mme_link() // echo js getting link code
    {
        ...
        $href='http://m.me/'.$value.'?ref='.urlencode($plugin_data[0]["reference"]);
    ...
    public function checkbox_plugin_list()
    {
    ...
    public function checkbox_plugin_list_data()
    {
      ...
      $domain_name       = trim($this->input->post("search_domain_name",true));
      $display_columns = array("#",'id','domain_name','page_name','domain_code','actions','domain_code','reference','created_at','label_names');
      ...
      {   
          ...
          $info[$i]['actions'] = '<div style="min-width:100px;"><a class="btn btn-circle btn-outline-warning" href="'.base_url()."messenger_bot_enhancers/checkbox_plugin_edit/".$info[$i]['id'].'" title="'.$this->lang->line('Edit').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-edit"></i></a> &nbsp;';
    ...
    public function checkbox_plugin_add()
    {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(213,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function checkbox_plugin_edit($id=0)
    {
        if($id==0) exit();
        if($this->session->userdata('user_type') != 'Admin' && !in_array(213,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function send_to_messenger_list()
    {
    ...
    public function send_to_messenger_list_data()
    {
        ...
        for($i=0;$i<count($info);$i++)
        {
            ...
            $info[$i]['actions'] = '<div style="min-width:100px;"><a class="btn btn-circle btn-outline-warning" href="'.base_url()."messenger_bot_enhancers/send_to_messenger_edit/".$info[$i]['id'].'" title="'.$this->lang->line('Edit').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-edit"></i></a> &nbsp;';
            $info[$i]['actions'] .= '<a class="btn btn-circle btn-outline-danger delete_campaign" href="#" title="'.$this->lang->line('delete').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-trash-alt"></i></a>';
            $info[$i]['actions'] .= "</div><script>$('[data-toggle=\"tooltip\"]').tooltip();</script>";
    ...
    public function send_to_messenger_add()
    {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(214,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function send_to_messenger_edit($id=0)
    {
        if($id==0) exit();
        if($this->session->userdata('user_type') != 'Admin' && !in_array(214,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function mme_link_list_data()
    {
        ...
        $pagename = trim($this->input->post("search_page_id",true));
        $search_ref_name = trim($this->input->post("search_ref_name",true));
        $display_columns = array("#",'id','page_name','link_code','actions','link_code','reference','created_at','label_names');
        ...
        for($i=0;$i<count($info);$i++)
        {
            ...
            $info[$i]["page_name"] = "<a data-toggle='tooltip' data-original-title='".$this->lang->line('Visit Page')."' target='_BLANK' href='https://facebook.com/".$info[$i]["fb_page_id"]."'>".$info[$i]["page_name"]."</a>";
            $info[$i]['created_at'] = date('jS F y', strtotime($info[$i]['created_at']));
            $info[$i]['actions'] = '<a class="btn btn-circle btn-outline-warning" href="'.base_url()."messenger_bot_enhancers/mme_link_edit/".$info[$i]['id'].'" title="'.$this->lang->line('Edit').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-edit"></i></a> &nbsp;';
            $info[$i]['actions'] .= '<a class="btn btn-circle btn-outline-danger delete_campaign" href="#" title="'.$this->lang->line('delete').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-trash-alt"></i></a>';
    ...
    public function mme_link_js_code()
    {
       ...
       $str2='http://m.me/'.$value.'?ref='.urlencode($reference);
    ...
    public function mme_link_add_action()
    {
        ...
        $value=($username!="")?$username:$fb_id;
        $js_code2='http://m.me/'.$value.'?ref='.urlencode($reference);
    ...
    public function mme_link_edit($id=0)
    {
        if($id==0) exit();
        if($this->session->userdata('user_type') != 'Admin' && !in_array(215,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function customer_chat_plugin_list() // customer chat plugin
    {
    ...
    public function customer_chat_plugin_list_data()
    {
        ...
        $pagename = trim($this->input->post("search_page_id",true));
        $search_ref_name = trim($this->input->post("search_ref_name",true));
        $display_columns = array("#",'id','domain_name','page_name','domain_code','domain_code','actions','add_date','language');
        ...
        for($i=0;$i<count($info);$i++)
        {
            ...
            $info[$i]['actions'] = '<a class="btn btn-circle btn-outline-warning" href="'.base_url()."messenger_bot_enhancers/customer_chat_edit/".$info[$i]['id'].'" title="'.$this->lang->line('Edit').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-edit"></i></a> &nbsp;';
    ...
    public function customer_chat_add()
    {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(217,$this->module_access))
        redirect('home/login_page', 'location');
    ...
    public function wp_plugin2($page='',$auto_id='')
    {
        ...
        $xdata=$this->basic->get_data("messenger_bot_engagement_2way_chat_plugin",array("where"=>array("id"=>$auto_id,"facebook_rx_fb_user_info_id"=>$this->session->userdata("facebook_rx_fb_user_info"))));
        if(!isset($xdata[0])) exit();
    ...
    public function customer_chat_edit($id=0)
    {
    ...
    public function activate()
    {
        ...
        (
            0 => "
            CREATE TABLE IF NOT EXISTS `messenger_bot_broadcast` (
              ...
              `notification_type` enum('REGULAR','SILENT_PUSH','NO_PUSH') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'REGULAR',
              `message` mediumtext COLLATE utf8mb4_unicode_ci NOT NULL,
              `buttons` longtext COLLATE utf8mb4_unicode_ci NOT NULL,
              `creative_id` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL,
              `broadcast_id` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL,
              ...
              `posting_status` enum('SCHEDULED','IN_PROGRESS','FINISHED','CANCELED') COLLATE utf8mb4_unicode_ci DEFAULT 'IN_PROGRESS',
              `is_autopost` enum('0','1') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0' COMMENT 'rss post',
              PRIMARY KEY (`id`),
            ...
            CREATE TABLE IF NOT EXISTS `messenger_bot_broadcast_serial` (
              `id` int(11) NOT NULL AUTO_INCREMENT,
              `user_id` int(11) NOT NULL,
              `broadcast_type` enum('Non Promo','24H Promo') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Non Promo',
            ...
            11=> "ALTER TABLE `messenger_bot_drip_campaign` ADD `message_tag` VARCHAR(255) NOT NULL AFTER `timezone`;"
        ); 
    ...
    {        
        ...
        $sql=array
        (       
          0=> "DROP TABLE IF EXISTS `messenger_bot_broadcast`;",
          1=> "DROP TABLE IF EXISTS `messenger_bot_broadcast_serial`;",
          2=> "DROP TABLE IF EXISTS `messenger_bot_broadcast_serial_send`;",
          3=> "DROP TABLE IF EXISTS `messenger_bot_drip_campaign`;",
          4=> "DROP TABLE IF EXISTS `messenger_bot_drip_campaign_assign`;",
          5=> "DROP TABLE IF EXISTS `messenger_bot_drip_report`;",
          6=> "DROP TABLE IF EXISTS `messenger_bot_engagement_2way_chat_plugin`;",
          7=> "DROP TABLE IF EXISTS `messenger_bot_engagement_checkbox`;",
          8=> "DROP TABLE IF EXISTS `messenger_bot_engagement_checkbox_reply`;",
          9=> "DROP TABLE IF EXISTS `messenger_bot_engagement_mme`;",
          10=>"DROP TABLE IF EXISTS `messenger_bot_engagement_send_to_msg`;"
        );  
