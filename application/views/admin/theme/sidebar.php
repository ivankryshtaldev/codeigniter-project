    ...
    <ul class="sidebar-menu">
      ...
      <?php
        ...
        {              
            ...
            if($single_menu['header_text']!=''){
              $menu_html .= '<li class="menu-header">'.$this->lang->line($single_menu['header_text']).'</li>';
            }
            $extraText='';
            ...
            $tour_prop = ($single_menu['url']=='messenger_bot') ? 'data-name="startStep_2"' : '';
            $menu_html .= "<li class='".$dropdown_class1."' $tour_prop><a {$parent_newtab} href='".$site_url1.$single_menu['url']."' class='nav-link ".$dropdown_class2."'><i class= '".$single_menu['icon']."'></i> <span>".$this->lang->line($single_menu['name']).$extraText."</span></a>"; 
        ...
        if($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type')=='Member')
        {
          ...
          echo '<li class="menu-header">'.$this->lang->line("ADD ON").'</li>';
          $validate_url = preg_match('%^(?:(?:https?|ftp)://)(?:\S+(?::\S*)?@|\d{1,3}(?:\.\d{1,3}){3}|(?:(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)(?:\.(?:[a-z\d\x{00a1}-\x{ffff}]+-?)*[a-z\d\x{00a1}-\x{ffff}]+)*(?:\.[a-z\x{00a1}-\x{ffff}]{2,6}))(?::\d+)?(?:[^\s]*)?$%iu
', $this->session->userdata('custom_module')); 
        if (!empty($this->session->userdata('custom_module')) && ($validate_url === 1)) {
          echo'
          <li class="">
            <a href="'.$this->session->userdata('custom_module').'" class="nav-link" target="_blank"><i class="fa fa-plug"></i> <span>'
          .$this->lang->line("Custom Module").
          ' </a>
          </li>';
        }
        else {
          echo'
          <li class="">
            <a href="#" class="nav-link"><i class="fa fa-plug"></i> <span>'
          .$this->lang->line("Custom Module").
          ' </a>
          </li>';
        }
        echo'
          <li class="">
            <a href="'.base_url("my_files").'" class="nav-link"><i class="fa fa-folder"></i> <span>'
          .$this->lang->line("My Files").
          ' </a>
          </li>';
      ?>
    </ul>
...
(
  ...
  base_url("json_api_connector/index")=>base_url("messenger_bot"),
