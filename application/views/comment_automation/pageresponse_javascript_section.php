<script>
  ...
  $(document).on('click','.enable_page_response',function(e){
    e.preventDefault();
  	$("#pageresponse_private_message_offensive_words").emojioneArea({
  		autocomplete: false,
  ...
  $(document).on('click','#pageresponse_add_more_button',function(){
    ...
    /** Load Emoji For Filter Word when click on add more button **/
		...
		$("#pageresponse_filter_message_"+pageresponse_content_counter).emojioneArea({
      		autocomplete: false,
			pickerPosition: "bottom"
   	 	});
  ...
  $(document).on('change','input[name=pageresponse_message_type]',function(){    
    if($("input[name=pageresponse_message_type]:checked").val()=="generic")
    {
      ...
      /*** Load Emoji for generic message when clicked ***/
	    ...
	    $("#pageresponse_generic_message, #pageresponse_generic_message_private").emojioneArea({
    		autocomplete: false,
		    pickerPosition: "bottom"
 		  });
    ...
    else 
    {
      ...
      /*** Load Emoji When Filter word click , by defualt first textarea are loaded & No match found field****/
	    ...
	    $("#pageresponse_comment_reply_msg_1, #pageresponse_filter_message_1, #pageresponse_nofilter_word_found_text, #pageresponse_nofilter_word_found_text_private").emojioneArea({
    		autocomplete: false,
		    pickerPosition: "bottom"
 	 	  });
    }
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        if(response.is_delete_offensive == 'hide')
        {
          ...
          $("#pageresponse_edit_delete_offensive_comment_keyword").html(response.offensive_words);
          $("#pageresponse_edit_private_message_offensive_words").html(response.private_message_offensive_words);
	  
	      /**	make the emoji loads div id in a string for selection . This is the first add. **/
		    emoji_load_div_list=emoji_load_div_list+"#pageresponse_edit_private_message_offensive_words";
        ...
        if(response.reply_type == 'generic')
        {
          ...
          $("#pageresponse_edit_generic_message").html(auto_reply_text_array[0]['comment_reply']); 
          $("#pageresponse_edit_generic_message_private").html(auto_reply_text_array[0]['private_reply']);
		
    			/** Add generic reply textarea id into the emoji load div list***/
    			emoji_load_div_list=emoji_load_div_list+", #pageresponse_edit_generic_message, #pageresponse_edit_generic_message_private";
        ...
        else
        {
          ...
          $("#pageresponse_edit_nofilter_word_found_text").html(pageresponse_edit_nofilter_word_found_text[0]['comment_reply']);
          $("#pageresponse_edit_nofilter_word_found_text_private").html(pageresponse_edit_nofilter_word_found_text[0]['private_reply']);
		
		     /**Add no match found textarea into emoji load div list***/
		     emoji_load_div_list=emoji_load_div_list+", #pageresponse_edit_nofilter_word_found_text, #pageresponse_edit_nofilter_word_found_text_private";
          ...
          for(var i = 0; i < auto_reply_text_array.length; i++) {
            ...
            $("#pageresponse_edit_filter_message_"+j).html(unscape_reply_text);
            // added by mostofa 25-04-2017
            var unscape_comment_reply_text = auto_reply_text_array[i]['comment_reply_text'];
            $("#pageresponse_edit_comment_reply_msg_"+j).html(unscape_comment_reply_text);
		  
		        emoji_load_div_list=emoji_load_div_list+", #pageresponse_edit_filter_message_"+j+", #pageresponse_edit_comment_reply_msg_"+j;
  ...
  $(document).on('click','#pageresponse_edit_add_more_button',function(){
    ...
    /** Load Emoji For Filter Word when click on add more button during Edit**/
		...
		$("#pageresponse_edit_filter_message_"+pageresponse_edit_content_counter).emojioneArea({
    	autocomplete: false,
			pickerPosition: "bottom"
 	 	});
  ...
  $(document).on('change','input[name=pageresponse_edit_message_type]',function(){    
    ...
    else 
    {
      ...
      $("#pagerespo_edit_comment_reply_msg_1, #pageresponse_edit_filter_message_1, #pageresponse_edit_nofilter_word_found_text, #pageresponse_edit_nofilter_word_found_text_private").emojioneArea({
  		  autocomplete: false,
	      pickerPosition: "bottom"
	 	    });
